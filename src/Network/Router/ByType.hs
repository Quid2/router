...
newRouter msgBus = do
  state <- newState
  bus <- onTypedMsg msgBus "Type" (\msgType msgBody -> forward (const True) state msgType msgBody)
  return $ router (Proxy::Proxy (ByType ())) (handle state bus) (reportTxt state) (reportTyped state)

    where

      reportTyped state = do
         r <- typedBLOB . ByTypeReport . map (\(t,e) -> (t,asClientReport e)) <$> entries state
